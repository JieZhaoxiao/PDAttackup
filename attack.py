...
from tools.eval_asr import model_selection
import ssl
...
def pdattackup(
    ...
    classifier = model_selection(model_name).eval()
    ...
    with open('Text/qwen3_vl/15_tokens.txt', 'r', encoding='utf-8') as file:
        descriptions = file.readlines()

    if len(descriptions) != 1000:
        raise ValueError("Descriptions file must contain exactly 1000 lines.")

    target_obj = imagenet_label.refined_Label[label.item()]
    
    full_description = descriptions[img_index].strip()
    
    import re
    context_str = re.compile(re.escape(target_obj), re.IGNORECASE).sub("", full_description).strip()
    context_str = re.sub(r'^\s*[,;:]?\s*', '', context_str)
    
    if not context_str:
        context_str = "on a solid background"
    
    print(f"\n[Log] Image {img_index} - Target Object (y): \"{target_obj}\"")
    print(f"[Log] Image {img_index} - Context Candidates (C): \"{context_str}\"\n")
    
    base_prompt = target_obj
    target_prompt = context_str
    prompt = [base_prompt + " " + target_prompt] * 2

    true_label_tokens = model.tokenizer.encode(base_prompt)
    prompt_tokens = model.tokenizer.encode(prompt[0])
    ...
    for _, _ in enumerate(pbar):
        ...
        base_prompt_tokens = model.tokenizer.encode(base_prompt)
        len_base = len(base_prompt_tokens) - 2
        total_len = len(prompt_tokens) - 2
        len_context = total_len - len_base
        
        cgad_loss = torch.tensor(0.0).cuda()
        
        if len_context > 0:
            context_start_idx = 1 + len_base
            context_end_idx = 1 + total_len
            context_activations = before_attention_map[:, :, context_start_idx : context_end_idx]
            averaged_activations = context_activations.mean(dim=1) 
            max_scores, best_indices = torch.max(averaged_activations, dim=1)
            c_star_offset = best_indices[0].item()
            c_star_idx = context_start_idx + c_star_offset
            window_start = max(context_start_idx, c_star_idx - 1)
            window_end = min(context_end_idx, c_star_idx + 2)
            if _ == 0:
                decoded_target = model.tokenizer.decode(prompt_tokens[1 : 1 + len_base])
                decoded_anchor = model.tokenizer.decode(prompt_tokens[window_start : window_end])
                print(f"[CGAD V2 Regional Anchor Triggered] Translating `{decoded_target}` into the spatial physics of `{decoded_anchor}` (Tokens: {prompt_tokens[window_start : window_end]})")
            baseline_y = before_attention_map[:, :, 1 : 1 + len_base].mean(dim=2, keepdim=True)
            baseline_c_star = before_attention_map[:, :, window_start : window_end].mean(dim=2, keepdim=True)
            live_y = after_attention_map[:, :, 1 : 1 + len_base].mean(dim=2, keepdim=True)
            live_c_star = after_attention_map[:, :, window_start : window_end].mean(dim=2, keepdim=True)
            cgad_loss = F.mse_loss(live_y, baseline_c_star.detach()) + F.mse_loss(live_c_star, baseline_y.detach())
        ...
        weighted_cgad_loss = cgad_loss * args.cross_attn_loss_weight

        self_attn_loss = controller.loss * args.self_attn_loss_weight

        loss = self_attn_loss + attack_loss + weighted_cgad_loss
