...
from cross_attention_control import AttentionControlEdit
import attack_correct as attack
from PIL import Image
import numpy as np
import os
import glob
from tools.eval_asr import model_transfer
import random
import sys
...
parser.add_argument('--save_dir', default="output/resnet_qwen3_vl_correct/15", type=str,
                    help='Where to save the adversarial examples, and other results')
parser.add_argument('--images_root', default="imagenet_compatible/images", type=str,
                    help='The clean images root directory')
parser.add_argument('--label_path', default="imagenet_compatible/labels.txt", type=str,
...
def run_diffusion_attack(image, label, diffusion_model, diffusion_steps, guidance=2.5,
                         ...
                         model_name="inception", start_step=15, iterations=30, img_index=0, args=None):
    controller = AttentionControlEdit(diffusion_steps, self_replace_steps, args.res)

    adv_image, clean_acc, adv_acc = attack.pdattackup(diffusion_model, label, controller,
                                                                  num_inference_steps=diffusion_steps,
                                                                  guidance_scale=guidance,
                                                                  image=image,
                                                                  save_path=save_dir, res=res, model_name=model_name,
                                                                  start_step=start_step,
                                                                  iterations=iterations, img_index=img_index, args=args)
    ...
    for ind, image_path in enumerate(all_images):
        ...
        adv_image, clean_acc, adv_acc = run_diffusion_attack(tmp_image, label[ind:ind + 1],
                                                             ...
                                                             save_dir=os.path.join(save_dir,
                                                                                   str(ind).rjust(4, '0')), img_index=ind, args=args)
...
print(f"Program finished, total elapsed time: {total_time:.2f} seconds")
