...
def diffattack(
    ...
    target_prompt = descriptions[img_index].strip()
    prompt = [target_prompt] * 2
    ...
    for _, _ in enumerate(pbar):
        ...
        before_true_label_attention_map = before_attention_map[:, :, 1: len(target_label) - 1]
        after_true_label_attention_map = after_attention_map[:, :, 1: len(target_label) - 1]
